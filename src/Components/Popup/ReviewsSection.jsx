...
const ReviewsSection = ({ reviews, ownerReviews, userId, productId, addReview, setReviews }) => {
  ...
  const handleSaveClick = async (reviewId) => {
    ...
    try {
      const response = await fetch(`https://petflix-backend-620z.onrender.com/api/PR_/${reviewId}`, {
        method: 'PUT',
        headers: {
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`https://petflix-backend-620z.onrender.com/api/PR_/product/${productId}`);
      if (response.ok) {

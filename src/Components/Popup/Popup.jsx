...
const Popup = ({
  ...
  useEffect(() => {
    fetch('https://petflix-backend-620z.onrender.com/api/Animals')
      ...
      .then(data => {
        ...
        if (currentAnimal) {
          fetch(`https://petflix-backend-620z.onrender.com/api/User/${currentAnimal.userId}`)
            .then(response => response.json())
    ...
    fetch('https://petflix-backend-620z.onrender.com/api/Products')
      ...
      .then(data => {
        ...
        if (currentProduct) {
          fetch(`https://petflix-backend-620z.onrender.com/api/User/${currentProduct.userId}`)
            .then(response => response.json())
            .then(userData => setUserData(userData))
            .catch(error => console.error('Error fetching user data:', error));
        }
  ...
  const fetchItemStatus = async (itemId, itemType) => {
    ...
    try {
      ...
      const favoriteResponse = await axios.get(`https://petflix-backend-620z.onrender.com/api/Favorite/${userId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  useEffect(() => {
    if (currentItemDetails) {
      ...
      const endpoint = isAnimal
        ? `https://petflix-backend-620z.onrender.com/api/AR_/animal/${currentItemId}`
        : `https://petflix-backend-620z.onrender.com/api/PR_/product/${currentItemId}`;
      ...
      fetch(endpoint)
        ...
        .then(reviewData => {
          const reviewsWithReviewer = reviewData.map(async (review) => {
            try {
              const userResponse = await fetch(`https://petflix-backend-620z.onrender.com/api/User/${review.reviewerId}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const apiUrl = `https://petflix-backend-620z.onrender.com/api/${derivedItemType}/${currentItemId}`;
  ...
  const handleSaveClick = async (reviewId) => {
    try {
      ...
      const response = await fetch(`https://petflix-backend-620z.onrender.com/api/${endpoint}/${reviewId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const confirmDeleteReview = async () => {
    ...
    try {
      const endpoint = isAnimal ? 'AR_' : 'PR_';
      const response = await fetch(`https://petflix-backend-620z.onrender.com/api/${endpoint}/${reviewToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleAddReview = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://petflix-backend-620z.onrender.com/api/${endpoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`

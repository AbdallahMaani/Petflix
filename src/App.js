...
function App() {
    ...
    const fetchData = async () => {
      try {
        const [usersRes, animalsRes, productsRes, feedbacksRes, animalReviewsRes, productReviewsRes] = await Promise.all([
          ...
          axios.get('https://petflix-backend-620z.onrender.com/api/Feedback'),
          axios.get('https://petflix-backend-620z.onrender.com/api/AR_'),
          axios.get('https://petflix-backend-620z.onrender.com/api/PR_'),
        ]);
        ...
        for (const user of usersRes.data) {
          try {
            const cartRes = await axios.get(`https://petflix-backend-620z.onrender.com/api/Carts/${user.userId}`);
            cartsData[user.userId] = cartRes.data || null;
          } catch (cartErr) {
            cartsData[user.userId] = null;
          }
          const ordersRes = await axios.get(`https://petflix-backend-620z.onrender.com/api/Order/user/${user.userId}`);
  ...
  const handleDelete = async (type, id) => {
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/${type}/${id}`);
      if (type === 'User') {
  ...
  const handleDeleteReview = async (type, reviewId) => {
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/${type}/${reviewId}`);
  ...
  const handleDeleteCart = async (userId, cartId) => {
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/Carts/${cartId}`);
      setCarts(prev => ({
        ...prev,
        [userId]: null,
      }));

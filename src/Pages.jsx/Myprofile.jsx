...
const [popupReviews, setPopupReviews] = useState([]);
    ...
    const fetchUserDataAndReviews = async () => {
        ...
        try {
            const [
                ...
                fetch(`‏https://localhost:7007/api/User/${userIdToFetch}`),
                fetch('‏https://localhost:7007/api/Animals'),
                fetch('‏https://localhost:7007/api/Products'),
                fetch(`‏https://localhost:7007/api/Favorite/${userIdToFetch}`),
                fetch('‏https://localhost:7007/api/PR_'),
                fetch('‏https://localhost:7007/api/AR_'),
            ]);
            ...
            const reviewersPromises = allReviewerIds.map((reviewerId) =>
                fetch(`‏https://localhost:7007/api/User/${reviewerId}`).then((res) => res.json())
            );
  ...
  const handleRemoveProfilePic = async () => {
    try {
      ...
      const response = await fetch(`‏https://localhost:7007/api/User/${user.userId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ...user, 
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('‏https://localhost:7007/api/AR_', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setSelectedAnimal('');
      setReviewContent('');
      const animalReviewsResponse = await fetch('‏https://localhost:7007/api/AR_');
  ...
  const handleProductReviewSubmit = async (e) => {
    ...
    try {
      ...
      const productReviewsResponse = await fetch('‏https://localhost:7007/api/PR_');
  ...
  const handleSave = async (dataToUpdate, endpoint, setState, setIsEditingState) => {
    ...
    try {
      const response = await fetch(`‏https://localhost:7007/api/User/${user.userId}${endpoint}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
const handleConfirmDelete = async () => {
  try {
    ...
    const response = await axios.delete(`‏https://localhost:7007/api/User/${user.userId}`, {
      headers: {
  ...
  const toggleFavorite = async (id, type, isCurrentlyFavorited) => {
    ...
    try {
      if (isCurrentlyFavorited && existingFav) {
        ...
        const response = await axios.delete(`‏https://localhost:7007/api/Favorite?userId=${userId}&itemId=${id}`);
        ...
        const response = await axios.post('‏https://localhost:7007/api/Favorite', favoriteData);
  ...
  try {
    const endpoint = itemType === 'animal' 
      ? `‏https://localhost:7007/api/Animals/${itemId}`
      : `‏https://localhost:7007/api/Products/${itemId}`;
    ...
    let reviewsData = [];
    if (itemType === 'product') {
      const reviewsResponse = await axios.get(`‏https://localhost:7007/api/PR_/product/${itemId}`);
      reviewsData = reviewsResponse.data;
    } else if (itemType === 'animal') {
      const reviewsResponse = await axios.get(`‏https://localhost:7007/api/AR_/animal/${itemId}`);
      reviewsData = reviewsResponse.data;
    }

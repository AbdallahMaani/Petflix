...
const Cart = () => {
      ...
      try {
        ...
        const itemsWithDetails = await Promise.all(
          cartData.cartItems.map(async (item) => {
            ...
            let ownerResponse;
            try {
              itemResponse = await axios.get(`https://petflix-backend-620z.onrender.com/api/Animals/${item.itemId}`);
            } catch (animalErr) {
              console.error(`Failed to fetch animal details for item ${item.itemId}:`, animalErr);
              try {
                itemResponse = await axios.get(`https://petflix-backend-620z.onrender.com/api/Products/${item.itemId}`);
              } catch (productErr) {
            ...
            try {
              ownerResponse = await axios.get(`https://petflix-backend-620z.onrender.com/api/User/${itemResponse.data.userId}`);
            } catch (ownerErr) {
  ...
  const handleQuantityChange = async (cartItemId, action) => {
    ...
    try {
      await axios.put(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}/items/${cartItemId}`, {
  ...
  const handleConfirmOrder = async ({ includeDelivery, tip }) => {
    try {
      ...
      if (response.status === 201) {
        await axios.delete(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}/items`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleViewItem = async (item) => {
    try {
      const endpoint = item.itemId && (await axios.get(`https://petflix-backend-620z.onrender.com/api/Animals/${item.itemId}`).catch(() => axios.get(`https://petflix-backend-620z.onrender.com/api/Products/${item.itemId}`))).then(res => res.data);
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}/items/${itemToDelete}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleRemoveFromCart = async (itemId) => {
    ...
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}/items/${cartItem.cartItemId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleAddToCart = async (itemId, quantity) => {
    try {
      ...
      await axios.post(
        `https://petflix-backend-620z.onrender.com/api/Carts/${userId}/items`,
      ...
      const updatedCart = await axios.get(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}`);
  ...
  const confirmClearCart = async () => {
    try {
      await axios.delete(`https://petflix-backend-620z.onrender.com/api/Carts/${userId}/clear`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
